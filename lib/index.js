...
DocsSection.prototype.find = function(params, callback) {
	...
	// `keys` or `values` can be chosen only via internal call
	if (params.setKeysOrValues !== internalParam) {
		params.keys = false;
		params.values = true;
	}
...
DocsSection.prototype.count = function(findParams, callback) {
	...
	// filtering
	findParams.values = Boolean(findParams.filter);
	findParams.keys = !Boolean(findParams.filter);
	findParams.countOnly = internalParam;
	findParams.setKeysOrValues = internalParam;
